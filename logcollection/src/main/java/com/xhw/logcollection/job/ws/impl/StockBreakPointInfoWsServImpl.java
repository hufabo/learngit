...
import com.xhw.logcollection.job.ws.StockBreakPointInfoWsServ;
 ...
 */
@Service
public class StockBreakPointInfoWsServImpl implements StockBreakPointInfoWsServ {

    @Autowired
    private BusStockBreakPointInfoMapper stockBreakPointInfoMapper;
    ...
    public boolean reportStockBreakPointInfo() throws Exception {
        ...
        //      ->“2.3.4. 存量数据块断点写入接口”
        System.out.println("2.3.4. 存量数据块断点写入接口");
        BusSyncTb syncTb = new BusSyncTb();
        InfomationWriterDto dto = new InfomationWriterDto();
        List<BusStockBreakPointInfo> dataList = null;
        ...
        if(recentSyncTb==null){//获取所有数据审计的记录
            dataList = stockBreakPointInfoMapper.listSync(null);
        }else {//获取最新的数据审计记录
            paraMap.put("gxsj",recentSyncTb.getGxsj());
            paraMap.put("id",recentSyncTb.getTbbzw());
            dataList = stockBreakPointInfoMapper.listSync(paraMap);
        }
        ...
        if(dataList.size()>1000){//总数据量大于一千分批次上传
            ...
            for(int i = 1;i <= count;i++){
                ...
                PageInfo<BusStockBreakPointInfo> pageInfo = new PageInfo<BusStockBreakPointInfo>(dataList);
                dto.setStockBreakPointInfos(pageInfo.getList());
                String xmlDoc = XmlUtil.beanToXml(dto,InfomationWriterDto.class);
                rstMap = wsUtil.sync(ContextBean.InterfaceID.STOCKBREAKPOINT.toString(),"writeObjectRds",xmlDoc);
            ...
            dto.setStockBreakPointInfos(dataList);
            String xmlDoc = XmlUtil.beanToXml(dto,InfomationWriterDto.class);
            rstMap = wsUtil.sync(ContextBean.InterfaceID.STOCKBREAKPOINT.toString(),"writeObjectRds",xmlDoc);
            flag = rstMap.get("code").toString().equals("1")?true:false;
            syncTb.setRstCode(rstMap.get("code").toString());
