...
import com.xhw.logcollection.job.ws.IncrFileInfoWsServ;
 ...
 */
@Service
public class IncrFileInfoWsServImpl implements IncrFileInfoWsServ {

    @Autowired
    private BusIncrementFileInfoMapper increFileInfoMapper;
    ...
    public boolean reportIncrFileInfo() throws Exception {
        ...
        if(dataList.size()>1000){//总数据量大于一千分批次上传
            ...
            for(int i = 1;i <= count;i++){
                ...
                PageInfo<BusIncrementFileInfo> pageInfo = new PageInfo<BusIncrementFileInfo>(dataList);
                dto.setIncrementFileInfos(pageInfo.getList());
                String xmlDoc = XmlUtil.beanToXml(dto,InfomationWriterDto.class);
                rstMap = wsUtil.sync(ContextBean.InterfaceID.INCREFILE.toString(),"writeObjectRds",xmlDoc);
            ...
            dto.setIncrementFileInfos(dataList);
            String xmlDoc = XmlUtil.beanToXml(dto,InfomationWriterDto.class);
            rstMap = wsUtil.sync(ContextBean.InterfaceID.INCREFILE.toString(),"writeObjectRds",xmlDoc);
            syncTb.setRstCode(rstMap.get("code").toString());
            syncTb.setRstMsg(rstMap.get("message").toString());
