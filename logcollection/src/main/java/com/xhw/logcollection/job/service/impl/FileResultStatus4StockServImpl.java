 ...
 */
@Service
public class FileResultStatus4StockServImpl implements FileResultStatus4StockServ {
    ...
    public void syncFileStatus() throws Exception {
        ...
        List<BusStockFileInfo> busStockFileInfos = mapper.listByCondition(params);
        // 没有记录则退出该方法
        if(busStockFileInfos.size() == 0) return;

        // 保存文件名清单 用于 接口查询
        List<String> files = new ArrayList<>();
        // 缓存文件本地状态 用于 和接口返回的状态做对比
        Map<String, String> statusCache = new HashMap<>();
        for(BusStockFileInfo stockFileInfo:busStockFileInfos){
            files.add(stockFileInfo.getWjm());
            statusCache.put(stockFileInfo.getWjm(), stockFileInfo.getWjzt());
        }
        ...
        // TODO 返回测试数据
        String resultXML = FileUtil.readResourceFile("/datas/file_result_stock.xml");
        ...
        for (FileResultStatusDto.FileResultBody resultBody:resultBodies){
            ...
            if(Constant.STOCK_FILE_STUS_UPLOAD_BJ.equals(status)
                ...
                BusStockFileInfo info = new BusStockFileInfo();
                Date now = new Date();
                info.setWjm(fileName);
                info.setWjzt(Constant.STOCK_FILE_STUS_UPLOAD_BJ);
                info.setGxsj(now);
                info.setScbjsj(now);
                mapper.updateFileInfo(info);
            }
            ...
            if(Constant.STOCK_FILE_STUS_STORAGE.equals(status)){
                ...
                String oldFileName = JedisUtil.pushStockFileName(fileName);
                if(oldFileName != null){
        ...
        // 更新存量处理状态表
        stockDealStatusServ.updateRKWJS();
    }
}
