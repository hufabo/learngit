 ...
 */
@Service
public class ReportHeartbeatWsServImpl implements ReportHeartbeatWsServ {

    @Autowired
    private BusMonitorHeartbeatMapper heartbeatMapper;
    ...
    public boolean reportHeartbeat(BusMonitorHeartbeat bean) throws Exception {
        ...
        if(dataList.size()>1000){//总数据量大于一千分批次上传
            ...
            for(int i = 1;i <= count;i++){
                ...
                PageInfo<BusMonitorHeartbeat> pageInfo = new PageInfo<BusMonitorHeartbeat>(dataList);
                dto.setHeartbeats(pageInfo.getList());
                String xmlDoc = XmlUtil.beanToXml(dto,InfomationWriterDto.class);
                rstMap = wsUtil.sync(ContextBean.InterfaceID.HEARTBEAT.toString(),"writeObjectRds",xmlDoc);
