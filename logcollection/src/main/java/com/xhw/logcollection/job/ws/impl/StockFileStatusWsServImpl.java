...
import com.xhw.logcollection.job.ws.StockFileStatusWsServ;
 ...
 */
@Service
public class StockFileStatusWsServImpl implements StockFileStatusWsServ {
    ...
    public boolean reportStockFileStatus() throws Exception {
        ...
        if(dataList.size()>1000){//总数据量大于一千分批次上传
            ...
            for(int i = 1;i <= count;i++){
                ...
                PageInfo<BusStockDealStatus> pageInfo = new PageInfo<BusStockDealStatus>(dataList);
                dto.setStockDealStatuses(pageInfo.getList());
                String xmlDoc = XmlUtil.beanToXml(dto,InfomationWriterDto.class);
                rstMap = wsUtil.sync(ContextBean.InterfaceID.STOCKDEALSTATUS.toString(),"writeObjectRds",xmlDoc);
            ...
            dto.setStockDealStatuses(dataList);
            String xmlDoc = XmlUtil.beanToXml(dto,InfomationWriterDto.class);
            rstMap = wsUtil.sync(ContextBean.InterfaceID.STOCKDEALSTATUS.toString(),"writeObjectRds",xmlDoc);
            syncTb.setRstCode(rstMap.get("code").toString());
            syncTb.setRstMsg(rstMap.get("message").toString());
