...
@Api("增量数据断点")
@RestController
@RequestMapping("api/increfile")
public class BusIncrementFileInfoController {
    private Gson gson = new GsonBuilder().serializeNulls().create();

    @Autowired
    private BusIncrementFileInfoServ fileInfoServ;

    @ApiOperation(value = "根据条件获取增量文件列表", httpMethod = "POST")
    @ResponseBody
    ...
    public String list(@ApiParam(value = "Json字符串") @RequestBody String data){
        ...
        try{
            ...
            PageInfo<BusIncrementFileInfo> pageInfo = fileInfoServ.listByCondition(paraMap);
